...
ModelTuned_SMOTE.py
-------------------
Heat Vulnerability Risk Prediction — SMOTE Version

Based on ModelTuned_2Income.py with SMOTE added to address class imbalance.

Training approach:
    Trains on observed counties only (suppression_flag == 0).
    SMOTE (Synthetic Minority Oversampling Technique) is applied inside each
    training fold to generate synthetic high-risk county examples, addressing
    the 4.5% positive rate without introducing noisy labels.

    SMOTE is applied AFTER train/test split and ONLY to training data.
    Test folds are never touched by SMOTE — evaluation remains valid.

    Fairness sensitive attribute: income_group (persistent poverty binary)
    Source: USDA Rural Development Persistent Poverty County List
    1 = persistent poverty county, 0 = non-persistent poverty county

Key difference from ModelTuned_2Income.py:
    Class imbalance is handled by SMOTE rather than class_weight alone.
    class_weight sweep tests lighter options since SMOTE already balances.
    SMOTE parameters (k_neighbors) added to sweep for tuning.

Outputs (all in TunedOutputs_SMOTE/):
    Sweep CSVs:
      sweep_eps.csv
...
LABEL_COL        = "label"
SENSITIVE_COL    = "income_group"      # persistent poverty binary: 1 = persistent
HHI_RANKING_COL  = "OVERALL_RANK"
DATA_QUALITY_COL = "n_zctas_in_county"
SUPPRESSION_COL  = "suppression_flag"  # 0 = observed, 1 = suppressed
...
OUTPUT_DIR        = "TunedOutputs_SMOTE"

# SMOTE default — tuned in sweep
SMOTE_K_NEIGHBORS = 5
...
def split_observed_suppressed(df: pd.DataFrame):
    ...
    print(f"  Persistent poverty       : {train_df[SENSITIVE_COL].sum()} "
          f"({train_df[SENSITIVE_COL].mean():.1%})")
    return train_df, predict_df


def extract_arrays(df: pd.DataFrame):
    """Extract X, y, A, and metadata from a dataframe."""
...
def generate_cv_splits(y: np.ndarray) -> list:
    """Stratified 5-fold CV on observed counties only."""
...
def apply_smote(X_train: np.ndarray,
                y_train: np.ndarray,
                A_train: np.ndarray = None,
                k_neighbors: int = SMOTE_K_NEIGHBORS,
                sampling_ratio: float = 0.5):
    """
    Apply SMOTE to training data to oversample the minority (high-risk) class.

    SMOTE generates synthetic high-risk county examples by interpolating between
    existing high-risk counties in feature space. Applied only to training data —
    test folds are never touched.

    For the constrained model, A_train (sensitive attribute) must also be
    resampled. Synthetic samples are all positive class, so we randomly assign
    sensitive attributes from existing positive-class samples to preserve the
    group distribution among high-risk counties.

    Returns:
        X_res, y_res          — resampled features and labels
        A_res (if A provided) — resampled sensitive attributes
    """
    n_minority = (y_train == 1).sum()

    # SMOTE requires at least k_neighbors + 1 minority samples
    # Fall back to k=1 if too few positive examples
    ...
    smote = SMOTE(sampling_strategy=sampling_ratio, random_state=RANDOM_STATE, k_neighbors=k)
    X_res, y_res = smote.fit_resample(X_train, y_train)

    if A_train is None:
        return X_res, y_res

    # Reconstruct A_train for resampled data
    # Original samples keep their group; synthetic samples (all positive class)
    # get group assignments sampled from existing positive-class distribution
    n_synthetic  = len(X_res) - len(X_train)
    pos_mask     = y_train == 1
    A_res = np.concatenate([
        A_train,
        np.random.choice(A_train[pos_mask], size=n_synthetic, replace=True)
    ])
    return X_res, y_res, A_res
...
def tune_regularization(X_train: np.ndarray,
    ...
    Note: SMOTE is not applied inside the inner CV to keep tuning efficient.
    The regularization strength is robust to this simplification.
    """
    if c_grid is None:
    ...
    search   = GridSearchCV(base_model, {"C": c_grid}, cv=inner_cv,
...
def ensemble_predict_proba(mitigator: ExponentiatedGradient,
    ...
    for est, weight in zip(mitigator.predictors_, mitigator.weights_):
        ...
        if p.shape[1] == 1:
            raise ValueError(
                "predict_proba returned single column — training data likely "
                "has only one class. Check label distribution.")
        proba += weight * p[:, 1]
    return proba
...
def compute_tpr_disparity(y_true: np.ndarray,
                           y_pred: np.ndarray,
                           A: np.ndarray) -> float:
    """TPR disparity between persistent poverty and non-persistent counties."""
    tprs = []
...
def train_unconstrained(
    ...
    """
    Unconstrained logistic regression with SMOTE applied inside each fold.

    SMOTE workflow per fold:
      1. Split train/test
      2. Standardize (fit on train, apply to test)
      3. Apply SMOTE to training data only → balanced training set
      4. Tune C on SMOTE-augmented training data
      5. Train model on SMOTE-augmented training data
      6. Evaluate on original test data (no SMOTE)

    class_weight=None by default since SMOTE handles class balance.
    Setting class_weight='balanced' adds additional weighting on top of SMOTE.
    """
    all_predictions  = []
    all_coefficients = []
    all_suppressed   = []
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        ...
        if not tag:
            print(f"  SMOTE: {(y_train==1).sum()} → {(y_train_res==1).sum()} "
                  f"high-risk training examples")

        # Tune C on SMOTE-augmented training data
        best_C = tune_regularization(X_train_res, y_train_res,
                                     c_grid=c_grid,
...
def train_constrained(
    ...
    """
    Fairness-constrained logistic regression with SMOTE applied inside each fold.
    Sensitive attribute: income_group (persistent poverty binary).
    EqualizedOdds enforces TPR/FPR parity between persistent and
    non-persistent poverty counties.

    SMOTE workflow per fold:
      1. Split train/test
      2. Standardize
      3. Apply SMOTE — synthetic samples get sensitive attribute assignments
         sampled from existing positive-class group distribution
      4. Fit ExponentiatedGradient on SMOTE-augmented data
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        ...
        # Apply SMOTE — returns resampled X, y, and reconstructed A
        X_train_res, y_train_res, A_train_res = apply_smote(
            X_train, y_train, A_train, k_neighbors=k_neighbors
        )
        ...
        mitigator.fit(X_train_res, y_train_res, sensitive_features=A_train_res)
...
def generate_cdc_baseline(train_df: pd.DataFrame,
    ...
    """CDC equal-weights baseline evaluated on observed counties only."""
...
def run_hyperparameter_sweep(X, y, A, meta, splits,
                              X_suppressed=None, meta_suppressed=None):
    """
    Sweep four hyperparameters on observed counties only.

    Experiment 1 — eps: fairness tolerance in ExponentiatedGradient
    Experiment 2 — class_weight: None vs balanced (SMOTE handles main imbalance)
    Experiment 3 — C in constrained model base estimator
    Experiment 4 — SMOTE k_neighbors
    ...
    # SMOTE handles the main class imbalance so heavy weights aren't needed
    print("\n" + "="*60)
    print("SWEEP 2: class_weight (SMOTE handles main imbalance)")
    print("="*60)
    ...
    for lbl, cw in cw_options:
        ...
        preds, _ = train_unconstrained(
            ...
            X_suppressed, meta_suppressed,
            class_weight=cw,
            tag=f"sweep_cw_{lbl.replace(' ', '_').replace('(', '').replace(')', '')}"
        )
        auc, auc_std, f1, f1_std, disp = fold_summary(preds)
        cw_results.append({
    ...
    # ── Experiment 4: SMOTE k_neighbors ──────────────────────────────────────
    print("\n" + "="*60)
    print("SWEEP 4: SMOTE k_neighbors")
    print("="*60)
    print("  Controls how many neighbors SMOTE uses to generate synthetic samples.")
    print("  Smaller k = more local/conservative interpolation.")
    print("  Larger k = more diverse synthetic samples.")

    k_values  = [3, 5, 7, 10]
    k_results = []

    for k in k_values:
        print(f"  k_neighbors={k} ...", end=" ", flush=True)
        preds, _ = train_unconstrained(
            X, y, meta, splits,
            X_suppressed, meta_suppressed,
            class_weight=best_cw,
            k_neighbors=k,
            tag=f"sweep_smote_k{k}"
        )
        auc, auc_std, f1, f1_std, disp = fold_summary(preds)
        k_results.append({
            "k_neighbors": k, "mean_auc": auc, "std_auc": auc_std,
            "mean_f1": f1, "std_f1": f1_std,
            "mean_tpr_disparity": disp,
        })
        print(f"AUC={auc:.4f} | F1={f1:.4f} | disp={disp:.4f}")
    ...
    k_df.to_csv(os.path.join(OUTPUT_DIR, "sweep_smote_k.csv"), index=False)

    best_k_idx = k_df["mean_f1"].idxmax()
    best_k     = int(k_df.loc[best_k_idx, "k_neighbors"])
    print(f"  → sweep_smote_k.csv saved | best k_neighbors={best_k}")
    ...
    print(f"  best_k_neighbors   = {best_k}")

    return best_eps, best_cw, best_cw_label, best_constrained_C, best_k
...
def tune_threshold(predictions_df: pd.DataFrame,
    ...
    """
    Find the threshold that maximizes F1 subject to TPR disparity <= max_disparity
    between persistent and non-persistent poverty counties.
    """
    ...
    print(f"  TPR by income group at fair threshold:")
    for g, tpr in group_tprs.items():
        lbl = "persistent poverty" if g == 1 else "non-persistent poverty"
        print(f"    Group {g} ({lbl}): TPR={tpr:.4f}")
    ...
    pd.DataFrame([
        {"income_group": g,
         "label": "persistent_poverty" if g == 1 else "non_persistent",
         "tpr_at_fair_threshold": tpr}
        for g, tpr in group_tprs.items()
...
def main():
    ...
    print(f"SMOTE will oversample minority class inside each training fold")
    ...
    print("="*60)
    best_eps, best_cw, best_cw_label, best_constrained_C, best_k = \
        run_hyperparameter_sweep(X, y, A, meta, splits, X_sup, meta_sup)

    # 6. Final unconstrained model with best params
    print("\n" + "="*60)
    print(f"FINAL UNCONSTRAINED MODEL "
          f"(class_weight={best_cw_label}, k_neighbors={best_k})")
    print("="*60)
    ...
    print("\n" + "="*60)
    print(f"FINAL CONSTRAINED MODEL "
          f"(eps={best_eps}, C={best_constrained_C}, k_neighbors={best_k})")
    print("="*60)
    train_constrained(
    ...
    # 10. Summary
    print("\n" + "="*60)
    print("ALL OUTPUTS SAVED TO: TunedOutputs_SMOTE/")
    print("="*60)
    ...
    print("\n  Sweep CSVs:")
    for f in ["sweep_eps.csv", "sweep_class_weight.csv",
              "sweep_constrained_C.csv", "sweep_smote_k.csv"]:
        print(f"    {f}")
    print("\n  Threshold tuning:")
