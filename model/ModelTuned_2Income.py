...
ModelTuned.py
-------------
Heat Vulnerability Risk Prediction — Hyperparameter Tuning Module

Training approach:
    Trains on observed counties only (suppression_flag == 0).
    Suppressed counties are excluded from training because MICE-imputed labels
    proved incoherent — counties with fewer than 10 deaths were being labeled
    high-risk at 30-39% rates, destroying AUC from 0.91 to 0.58.
    Suppressed counties are predicted by the trained model as an external
    validation layer and saved separately.
...
SUPPRESSION_COL  = "suppression_flag"  # 0 = observed, 1 = suppressed (excluded from training)
...
OUTPUT_DIR        = "TunedOutputs_2Incomes"
...
def load_data(path: str = "../data/heat_risk_dataframe.csv") -> pd.DataFrame:
    ...
    # Drop missing labels — counties with no CDC WONDER match
    before = len(df)
...
def split_observed_suppressed(df: pd.DataFrame):
    ...
    Suppressed counties are excluded from training because their MICE-imputed
    labels are unreliable. They are predicted by the trained model as an
    ...
    print(f"  Persistent poverty (train): {train_df[SENSITIVE_COL].sum()} "
...
def generate_cv_splits(y: np.ndarray) -> list:
    ...
    Preserves the ~5.6/94.4 class balance across folds.
    """
...
def train_unconstrained(
    ...
    """
    Unconstrained logistic regression on observed counties.
    class_weight='balanced' compensates for 5.6/94.4 class imbalance.
    Suppressed counties are predicted by each fold's model for external validation.
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        ...
        # Apply to suppressed counties — labels unknown, used as external validation
...
def generate_cdc_baseline(train_df: pd.DataFrame,
    ...
    Applied to all counties for completeness.
    Evaluated on observed counties only since those are the ground-truth labels.
    """
...
def run_hyperparameter_sweep(X, y, A, meta, splits,
    ...
    Experiment 2 — class_weight: compensating for 5.6/94.4 imbalance
    ...
    # Best eps: highest F1 where disparity <= 0.10
    ...
    print("\n" + "="*60)
    print("SWEEP 2: class_weight — compensating for 5.6/94.4 imbalance")
    print("="*60)

    # Natural imbalance ratio: ~1237/74 ≈ 17x
    cw_options = [
        ("balanced",    "balanced"),
        ("{0:1,1:5}",   {0: 1, 1: 5}),
        ("{0:1,1:10}",  {0: 1, 1: 10}),
